...
**0. Peer-to-peer protocol sharing verification (~30m).**
After the May 20 late-afternoon peer-to-peer ship, verify end-to-end with two test accounts: (1) From Sofia's account, open a protocol → ⋯ → Send to someone → type OVH's email → confirm toast "Sent to OVH". (2) Switch to OVH's account → confirm the Library icon has a "1" badge → tap Library → see "Received" section with the protocol → tap row → confirm the three buttons (Stack / Replace / Save for later) → pick Stack → confirm toast and the protocol appears in OVH's active protocols with `source: 'shared'`. (3) Repeat with Replace and Save for later flows against fresh sends. (4) Verify the OVH device receives a Web Push notification ("Sofia sent you a protocol · {name} · N supplements") — requires OVH having notifications enabled and the `notify_protocol_sent` edge function to have auto-deployed.
